...
add_action( 'wp_nav_menu_item_custom_fields', 'daneshyar_add_megamenu_fields', 10, 4 );
function daneshyar_add_megamenu_fields( $item_id, $item, $depth, $args ) { ?>
	<div class="clear"></div>
	<div class="daneshyar-mega-menu-options">
		<p class="field-megamenu-status description description-wide">
			<label for="edit-menu-item-megamenu-status-<?php echo $item_id; ?>">
				<input type="checkbox" id="edit-menu-item-megamenu-status-<?php echo $item_id; ?>" class="widefat code edit-menu-item-megamenu-status" name="menu-item-daneshyar-megamenu-status[<?php echo $item_id; ?>]" value="enabled" <?php checked( $item->daneshyar_megamenu_status, 'enabled' ); ?> />
				<strong><?php esc_html_e( 'فعالسازی مگامنو', 'daneshyar' ); ?></strong>
			</label>
		</p>

		<p class="field-megamenu-columns description description-wide">
			<label for="edit-menu-item-megamenu-columns-<?php echo $item_id; ?>">
				<?php esc_html_e( 'تعداد ستون های مگامنو', 'daneshyar' ); ?>
				<select id="edit-menu-item-megamenu-columns-<?php echo $item_id; ?>" class="widefat code edit-menu-item-megamenu-columns" name="menu-item-daneshyar-megamenu-columns[<?php echo $item_id; ?>]">
					<option value="auto" <?php selected( $item->daneshyar_megamenu_columns, 'auto' ); ?>><?php esc_html_e( 'خودکار', 'daneshyar' ); ?></option>
					<option value="1" <?php selected( $item->daneshyar_megamenu_columns, '1' ); ?>>1</option>
					<option value="2" <?php selected( $item->daneshyar_megamenu_columns, '2' ); ?>>2</option>
					<option value="3" <?php selected( $item->daneshyar_megamenu_columns, '3' ); ?>>3</option>
					<option value="4" <?php selected( $item->daneshyar_megamenu_columns, '4' ); ?>>4</option>
				</select>
			</label>
		</p>


	</div><!-- .daneshyar-mega-menu-options-->
<?php }
...
// Dont duplicate me!
if( ! class_exists( 'DaneshyarFrontendWalker' ) ) {
	class DaneshyarFrontendWalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if( $depth === 0 && $this->menu_megamenu_status == "enabled" ) {
				...
				$output .= "\n{first_level}\n";
				$output .= "\n$indent<div class=\"daneshyar-megamenu-holder\" {megamenu_final_width}>\n<ul class='daneshyar-megamenu {megamenu_border}'>\n";
		...
		public function end_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if( $depth === 0  && $this->menu_megamenu_status == "enabled" ) {
				...
				if ( $this->menu_megamenu_width == "fullwidth" ) {
					$col_span = " col-span-12 daneshyar-megamenu-fullwidth";
				...
				if($this->menu_megamenu_thumbnail !=""){
					$output = str_replace( "{first_level}", "<div class='daneshyar-megamenu-wrapper {daneshyar_columns} columns-".$this->total_num_of_columns . $col_span . "' data-maxwidth='" . $this->menu_megamenu_maxwidth . "' style=background-image:url('". $this->menu_megamenu_thumbnail ."')><div class='row'>", $output );
				}else{
						$output = str_replace( "{first_level}", "<div class='daneshyar-megamenu-wrapper {daneshyar_columns} columns-".$this->total_num_of_columns . $col_span . "' data-maxwidth='" . $this->menu_megamenu_maxwidth . "' ><div class=''>", $output );
				}

				$output = str_replace( "{megamenu_final_width}", sprintf( 'style="width:%spx;" data-width="%s"', $wrapper_width, $wrapper_width ), $output );

				if ( $this->total_num_of_columns > $this->max_num_of_columns ) {
					$output = str_replace( "{megamenu_border}","daneshyar-megamenu-border", $output );
				} else {
				...
				foreach($this->submenu_matrix as $row => $columns) {
					...
					if( ( $row - 1 ) * $this->max_num_of_columns + $columns < $this->total_num_of_columns ) {
						$output = str_replace( "{row_number_".$row."}", "daneshyar-megamenu-row-columns-" . $columns . " daneshyar-megamenu-border", $output);
					} else {
						$output = str_replace( "{row_number_".$row."}", "daneshyar-megamenu-row-columns-" . $columns, $output);
					}
					$output = str_replace( "{current_row_".$row."}", "daneshyar-megamenu-columns-".$columns. " col-md-" . $layout_columns , $output );

					$output = str_replace( "{daneshyar_columns}", sprintf( 'daneshyar-columns-%s columns-per-row-%s', $columns, $columns ), $output );
				}
			} else {
				$output .= "$indent</ul>\n";
			}
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if( $depth === 0 ) {
				...
				$this->menu_megamenu_status = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_status', true );
				$this->menu_megamenu_width = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_width', true);
				$allowed_columns = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_columns', true );
				$this->menu_megamenu_thumbnail = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_thumbnail', true);
			...
			$this->menu_megamenu_title = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_title', true);
			$this->menu_megamenu_innertitle = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_innertitle', true);
			$this->menu_megamenu_widgetarea = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_widgetarea', true);
			$this->menu_megamenu_newlabel = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_newlabel', true);
			$this->menu_megamenu_hotlabel = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_hotlabel', true);
			$this->menu_megamenu_salelabel = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_salelabel', true);
			$this->menu_megamenu_icon = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_icon', true);
			$this->menu_megamenu_thumbnail = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_thumbnail', true);

			/* we are inside a mega menu */
			if( $depth === 1 && $this->menu_megamenu_status == "enabled" ) {

				if( get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_columnwidth', true) ) {
					$this->menu_megamenu_columnwidth = get_post_meta( $item->ID, '_menu_item_daneshyar_megamenu_columnwidth', true);
				} else {
					if ( $this->menu_megamenu_width == "fullwidth" ) {
				...
				if( $this->num_of_columns > $this->max_num_of_columns ) {
					...
					$output .= "\n</ul>\n<ul class=\"daneshyar-megamenu daneshyar-megamenu-row-".$this->num_of_rows." {row_number_".$this->num_of_rows."}\" {row_width_".$this->num_of_rows."}>\n";
				} else {
					$this->menu_megamenu_rowwidth_matrix[$this->num_of_rows] +=  floatval( $this->menu_megamenu_columnwidth ) / 100;
				}
				...
				if( !(
					...
					if ( ! empty( $this->menu_megamenu_thumbnail ) ) {
						$title_enhance = '<span class="daneshyar-megamenu-icon"><img src="' . $this->menu_megamenu_thumbnail . '" alt="thumbnail image"></span>';
					} elseif( ! empty( $this->menu_megamenu_icon ) ) {
						$title_enhance = '<span class="daneshyar-megamenu-icon"><i class="fa glyphicon ' . $this->menu_megamenu_icon .'"></i></span>';
					} else
					...
					if( $this->menu_megamenu_title != 'disabled' ) {
						$item_output .= "<div class='daneshyar-megamenu-title'>" . $heading . "</div>";
					} else {
						$item_output .= $heading;
					}
				}
				...
				if( $this->menu_megamenu_widgetarea &&
					...
					$item_output .= '<div class="daneshyar-megamenu-widgets-container second-level-widget">';
					ob_start();
				...
				if( is_active_sidebar( $this->menu_megamenu_widgetarea ) ) {
					$item_output .= '<div class="daneshyar-megamenu-widgets-container third-level-widget">';
					ob_start();
						dynamic_sidebar( $this->menu_megamenu_widgetarea );
				...
				if( $this->menu_megamenu_innertitle == 'enabled' && $this->menu_megamenu_status == "enabled") {
					$item_output .= "<div class='daneshyar-megamenu-title'>";
				}
				if ( ! empty( $this->menu_megamenu_thumbnail ) ) {
					$item_output .= '<a ' . $attributes . '><span class="daneshyar-megamenu-icon"><img src="' . $this->menu_megamenu_thumbnail . '" alt="thumbnail image"></span>';
				} elseif( ! empty( $this->menu_megamenu_icon ) ) {
					$item_output .= '<a ' . $attributes . '><span class="daneshyar-megamenu-icon text-menu-icon"><i class="fa glyphicon ' . $this->menu_megamenu_icon .'"></i></span>';
				} else
				if ( $depth !== 0 && $this->menu_megamenu_status == "enabled") {
				...
				if($this->menu_megamenu_newlabel == "enabled" || $this->menu_megamenu_hotlabel == "enabled" || $this->menu_megamenu_salelabel == "enabled"){
					$item_output .=  '<span class="daneshyar-menu-label">';
					if( $this->menu_megamenu_newlabel == "enabled") {
						$item_output .=  '<span class="daneshyar-menu-newlabel">'.esc_attr__('New','daneshyar').'</span>';
					}
					if( $this->menu_megamenu_hotlabel == "enabled" ) {
						$item_output .=  '<span class="daneshyar-menu-hotlabel">'.esc_attr__('Hot','daneshyar').'</span>';
					}
					if( $this->menu_megamenu_salelabel == "enabled" ) {
						$item_output .=  '<span  class="daneshyar-menu-salelabel">'.esc_attr__('Sale','daneshyar').'</span>';
					}
					$item_output .=  '</span>';
				}
			...
			if ( $this->menu_megamenu_status != "enabled" && ( ( strcasecmp( $item->attr_title, 'divider' ) == 0) ||
				...
				if( $depth === 0 && $args->has_children ) {
					if( $this->menu_megamenu_status == "enabled" ) {
						$class_names .= ' daneshyar-megamenu-menu';
					} else {
						$class_names .= ' daneshyar-dropdown-menu';
					}
				}

				if ( $depth === 1 ) {
					if( $this->menu_megamenu_status == "enabled" ) {
						$class_names .= ' daneshyar-megamenu-submenu';
						...
						$class_names .= ' daneshyar-dropdown-submenu';
					}
				}
	...
	}  // end DaneshyarFrontendWalker() class
}

// Don't duplicate me!
if( ! class_exists( 'daneshyarCoreMegaMenus' ) ) {

	class daneshyarCoreMegaMenus extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( ! empty( $item->_invalid ) ) {
				...
				$title = sprintf( esc_html__( '%s (Invalid)', 'daneshyar'), $item->title );
			} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				$classes[] = 'pending';
				/* translators: %s: title of menu item in draft status */
				$title = sprintf( esc_html__('%s (Pending)', 'daneshyar'), $item->title );
			}
	...
	} // end daneshyarCoreMegaMenus() class

}


// Don't duplicate me!
if( ! class_exists( 'daneshyarMegaMenu' ) ) {
	...
	class daneshyarMegaMenu{
		...
		/**
		 * Function to replace normal edit nav walker for daneshyar core mega menus
		 *
		 * @return string Class name of new navwalker
		 */
		function add_custom_fields() {

			return 'daneshyarCoreMegaMenus';
		...
		function save_custom_fields( $menu_id, $menu_item_db_id, $args ) {
			...
			foreach ( $field_name_suffix as $key ) {
				if( !isset( $_REQUEST['menu-item-daneshyar-megamenu-'.$key][$menu_item_db_id] ) ) {
					$_REQUEST['menu-item-daneshyar-megamenu-'.$key][$menu_item_db_id] = '';
				}

				$value = $_REQUEST['menu-item-daneshyar-megamenu-'.$key][$menu_item_db_id];
				update_post_meta( $menu_item_db_id, '_menu_item_daneshyar_megamenu_'.$key, $value );
			}
		}
		...
		function add_data_to_menu( $menu_item ) {
			...
			$menu_item->daneshyar_megamenu_status = get_post_meta( $menu_item->ID, '_menu_item_daneshyar_megamenu_status', true );

			$menu_item->daneshyar_megamenu_width = get_post_meta( $menu_item->ID, '_menu_item_daneshyar_megamenu_width', true );

			$menu_item->daneshyar_megamenu_columns = get_post_meta( $menu_item->ID, '_menu_item_daneshyar_megamenu_columns', true );

			$menu_item->daneshyar_megamenu_columnwidth = get_post_meta( $menu_item->ID, '_menu_item_daneshyar_megamenu_columnwidth', true );

			$menu_item->daneshyar_megamenu_title = get_post_meta( $menu_item->ID, '_menu_item_daneshyar_megamenu_title', true );

			$menu_item->daneshyar_megamenu_innertitle = get_post_meta( $menu_item->ID, '_menu_item_daneshyar_megamenu_innertitle', true );

			$menu_item->daneshyar_megamenu_newlabel = get_post_meta( $menu_item->ID, '_menu_item_daneshyar_megamenu_newlabel', true );

			$menu_item->daneshyar_megamenu_hotlabel = get_post_meta( $menu_item->ID, '_menu_item_daneshyar_megamenu_hotlabel', true );

			$menu_item->daneshyar_megamenu_salelabel = get_post_meta( $menu_item->ID, '_menu_item_daneshyar_megamenu_salelabel', true );

			$menu_item->daneshyar_megamenu_widgetarea = get_post_meta( $menu_item->ID, '_menu_item_daneshyar_megamenu_widgetarea', true );

			$menu_item->daneshyar_megamenu_icon = get_post_meta( $menu_item->ID, '_menu_item_daneshyar_megamenu_icon', true );

			$menu_item->daneshyar_megamenu_thumbnail = get_post_meta( $menu_item->ID, '_menu_item_daneshyar_megamenu_thumbnail', true );

			return $menu_item;

		}

	} // end daneshyarMegaMenu() class

}
$daneshyarMegaMenu = new daneshyarMegaMenu();
