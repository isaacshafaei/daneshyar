...
if(class_exists('Redux')){
    $header_layout =  daneshyar_settings('header_elementor');
}?>
<!DOCTYPE html>
...
<head>
    ...
    <?php if(daneshyar_settings('favie_icon')){ ?>
    <link rel="shortcut icon" href="<?php echo esc_url(daneshyar_settings('favie_icon')['url']); ?>">
    <?php } ?>
    <?php if(daneshyar_settings('favie_icon_retina')){ ?>
        <link rel="apple-touch-icon-precomposed" sizes="152x152" href="<?php echo esc_url(daneshyar_settings('favie_icon_retina')['url']); ?>">
    <?php } ?>
    <?php wp_head(); ?>
</head>
<body <?php body_class(); ?>>
...
if(!get_post_meta(get_the_ID( ), $perfix.'disable_header',true)){ 
 ...
 get_template_part('/inc/templates/header/loadings'); 
  do_action( 'daneshyar_before_body' ); ?>
...
if($header_layout != 'no-header'){ 
    ...
    if(!get_post_meta(get_the_ID(  ), $perfix.'disable_bredacrumb_page',true) && !is_front_page( )){
